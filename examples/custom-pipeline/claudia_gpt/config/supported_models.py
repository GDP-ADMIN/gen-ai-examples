...
from pydantic import BaseModel, BeforeValidator, ConfigDict, HttpUrl, SecretStr, ValidationError
...
class OpenAIModel(StrEnum):
    ...
    GPT_4O = "gpt-4o"
    GPT_4O_2024_11_20 = "gpt-4o-2024-11-20"  # Custom implementation for Claudia
...
# Claudia Adjustment: Add API Key mappings
API_KEY_MAP = {
    Provider.OPENAI: SecretStr(OPENAI_API_KEY),
    # Provider.ANTHROPIC: SecretStr(ANTHROPIC_API_KEY),
    # Provider.GOOGLE: SecretStr(GOOGLE_GENERATIVE_AI_API_KEY),
    # Provider.DEEPSEEK: SecretStr(DEEPSEEK_API_KEY),
    # Provider.GROQ: SecretStr(GROQ_API_KEY),
    # Provider.TOGETHER_AI: SecretStr(TOGETHER_API_KEY),
    # Provider.DEEPINFRA: SecretStr(DEEPINFRA_API_KEY),
    # Provider.TEI: SecretStr(TEI_API_KEY),
    # Provider.TGI: SecretStr(TGI_API_KEY),
    # Provider.VLLM: SecretStr(VLLM_API_KEY),
}


# Claudia Adjustment
def get_provider_api_key(provider: Provider) -> SecretStr:
    """Get the API key for the given provider.

    Args:
        provider (Provider): The provider to get the API key for.

    Returns:
        SecretStr: The API key for the provider.
    ...
    if api_key is None:
        raise ValueError(f"Unsupported provider when getting API key: {provider}")
    return api_key


def validate_model_name(model_name: str, provider: Provider) -> str:
    """Validate the model name is valid for the given provider.
